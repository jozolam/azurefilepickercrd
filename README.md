...
```sh
make docker-build docker-push IMG=<some-registry>/azurefilepickercrd:tag
```
...
```sh
make deploy IMG=<some-registry>/azurefilepickercrd:tag
```
...
```sh
make build-installer IMG=<some-registry>/azurefilepickercrd:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/azurefilepickercrd/<tag or branch>/dist/install.yaml
```
